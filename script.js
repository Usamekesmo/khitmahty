    ...
    db.collection('circleMembers').get().then(snap => {
        ...
        snap.forEach(d => {
            ...
            if (m.isActive !== false) {
                ...
                for (let i = 1; i <= 30; i++) {
                    const isTaken = takenJuz.includes(i);
                    const isRead = readJuz.includes(i);
                    const isExtra = extraReadJuz.includes(i);
                    
                    if (isRead && isExtra) {
                        readStatus[i] = '✅ مقروء (إضافي)';
                    } else if (isRead) {
                        readStatus[i] = '✅ مقروء';
                    } else if (isTaken) {
                        readStatus[i] = '❌ لم يقرأ';
                    } else {
                        readStatus[i] = '⏳ غير مأخوذ';
                    }
                }
                ...
                const row = {
                    ...
                    'الحلقة': m.circleId || '',
                    'الجزء المطلوب قراءته': m.currentJuz || m.selectedJuz || '',
                    'الجزء المقروء اليوم': readTodayJuz,
                    'حالة القراءة اليوم': hasReadToday ? '✅ قرأ اليوم' : '❌ لم يقرأ اليوم',
                    'إجمالي الأجزاء المقروءة': (m.totalPartsRead || 0) + (m.totalExtraJuz || 0),
                    'عدد الختمات': calcKhatmasFromParts(m.totalPartsRead || 0),
                    'الأجزاء المأخوذة': (m.takenJuzList || []).join('، ') || 'لا يوجد',
                    'جميع الأجزاء المقروءة': allReadJuz.length > 0 ? allReadJuz.join('، ') : 'لا يوجد',
                    'أيام الغياب': m.absenceCount || 0,
                    'الحالة': m.isActive ? 'نشط' : 'غير نشط'
                };
                
                for (let i = 1; i <= 30; i++) {
                    row[`الجزء ${i}`] = readStatus[i] || '⏳ غير مأخوذ';
                }
        ...
        showToast("✅ تم تصدير التقرير الكامل بنجاح", false);
    }).catch(e => {
        console.error(e);
...
function exportUsersSimpleCSV() {
    db.collection('circleMembers').get().then(snap => {
        ...
        showToast("✅ تم تصدير الملخص بنجاح", false);
    }).catch(e => {
        console.error(e);
        showToast("حدث خطأ أثناء التصدير", true);
    });
...
function exportUsersFullPDF() {
    ...
    db.collection('circleMembers').get().then(snap => {
        ...
        const headers = [
            ['#', 'الاسم', 'البريد الإلكتروني', 'تاريخ الانضمام', 'الجزء المطلوب', 
             'الجزء المقروء اليوم', 'حالة القراءة', 'إجمالي الأجزاء', 'الختمات', 
             'جميع الأجزاء المقروءة', 'أيام الغياب', 'الحالة']
        ];
        ...
        let tableHtml = `<table style="width:100%; border-collapse:collapse; font-size:10px;">
            <thead>
                <tr style="background:#1a4739; color:white;">
                    ...
                    <th style="padding:6px; border:1px solid #ddd;">الجزء المطلوب</th>
...
function buildUserListMessage(users, isActive) {
    ...
    users.forEach((u, i) => {
        ...
        message += `${i+1}. ${u.userName} | الجزء المطلوب: ${u.currentJuz || u.selectedJuz} | الجزء المقروء اليوم: ${readToday} | الأجزاء المقروءة: ${allReadJuz.length > 0 ? allReadJuz.join(', ') : 'لا يوجد'} | ${status}\n`;
    });
    ...
    for (const u of users) {
        ...
        usersTableRows += `
            <tr style="border-bottom:1px solid #ddd; ${hasReadToday ? 'background:#dcfce7;' : 'background:#fee2e2;'}">
                ...
                <td style="padding:8px;">${u.userEmail || '-'}</td>
                <td style="padding:8px;">${formatDate(u.joinedAt)}</td>
                <td style="padding:8px;">${u.currentJuz || u.selectedJuz}</td>
                <td style="padding:8px;">${readTodayJuz}</td>
                <td style="padding:8px;">${hasReadToday ? '✅ قرأ اليوم' : '❌ لم يقرأ اليوم'}</td>
                <td style="padding:8px;">${total}</td>
                <td style="padding:8px;">${khatmas}</td>
                <td style="padding:8px;">${mainParts}</td>
                <td style="padding:8px;">${extraParts.length > 0 ? extraParts.join(', ') : 'لا يوجد'}</td>
                <td style="padding:8px;">${takenJuz.length > 0 ? takenJuz.join(', ') : 'لا يوجد'}</td>
                <td style="padding:8px;">${allReadJuz.length > 0 ? allReadJuz.join(', ') : 'لا يوجد'}</td>
                <td style="padding:8px;">${u.totalExtraJuz || 0}</td>
                <td style="padding:8px;">${u.absenceCount || 0}</td>
                <td style="padding:8px;">${u.isActive ? '✅ نشط' : '❌ غير نشط'}</td>
            </tr>
        `;
    }
    
    const readCount = users.filter(u => {
        const lastRead = safeDateString(u.lastReadDate);
        return lastRead === today;
    }).length;
    ...
    cont.innerHTML = `
        <div class="report-section">
            ...
            <div class="report-stats">
                <div class="report-stat"><div class="label">👥 إجمالي المستخدمين</div><div class="value">${users.length}</div></div>
                <div class="report-stat"><div class="label">✅ قرأوا اليوم</div><div class="value" style="color:#16a34a;">${readCount}</div></div>
                <div class="report-stat"><div class="label">❌ لم يقرأوا اليوم</div><div class="value" style="color:#dc2626;">${users.length - readCount}</div></div>
                ...
                <div class="report-stat"><div class="label">📦 الأجزاء الإضافية</div><div class="value">${totalExtra}</div></div>
            </div>
        </div>
        <div class="report-section">
            ...
            <div style="overflow-x: auto;">
                <table style="width:100%; border-collapse:collapse; font-size:12px;">
                    <thead>
                        <tr>
                            <th style="text-align:right; padding:6px;">الاسم</th>
                            <th style="text-align:right; padding:6px;">البريد الإلكتروني</th>
                            <th style="text-align:right; padding:6px;">تاريخ الانضمام</th>
                            <th style="text-align:right; padding:6px;">الجزء المطلوب</th>
                            <th style="text-align:right; padding:6px;">الجزء المقروء اليوم</th>
                            <th style="text-align:right; padding:6px;">حالة القراءة اليوم</th>
                            <th style="text-align:right; padding:6px;">إجمالي الأجزاء</th>
                            <th style="text-align:right; padding:6px;">الختمات</th>
                            <th style="text-align:right; padding:6px;">الأجزاء الأساسية</th>
                            <th style="text-align:right; padding:6px;">الأجزاء الإضافية المقروءة</th>
                            <th style="text-align:right; padding:6px;">الأجزاء المأخوذة</th>
                            <th style="text-align:right; padding:6px;">جميع الأجزاء المقروءة</th>
                            <th style="text-align:right; padding:6px;">عدد الإضافية</th>
                            <th style="text-align:right; padding:6px;">أيام الغياب</th>
                            <th style="text-align:right; padding:6px;">الحالة</th>
                        </tr>
                    </thead>
...
async function completeDaily() {
    ...
    try {
        ...
        // الجزء الذي قرأه اليوم (الجزء الحالي قبل التحديث)
        ...
        // تحديث currentJuz إلى الجزء التالي، ولكن نحتفظ بقيمة readJuzToday لعرضه
        await ref.update({ 
            currentJuz: newJuz,
...
async function loadMyJuzList() {
    ...
    // الجزء المقروء اليوم (آخر جزء في قائمة readJuz)
    const today = getTodayString();
    const lastRead = safeDateString(currentMemberData.lastReadDate);
...
// ==================== دوال الاستماع للجزء (باستخدام Cloud Quran API) ====================

async function openAudioPlayer(juzNumber) {
    ...
    const juzSpan = document.getElementById('audioJuzNumber');
    const audio = document.getElementById('quranAudio');
    const source = document.getElementById('audioSource');
    const status = document.getElementById('audioStatus');
    
    juzSpan.textContent = juzNumber;
    status.textContent = '⏳ جاري تحميل الجزء كاملاً...';
    status.style.color = '#666';
    
    // استخدام Cloud Quran API للحصول على تلاوة الجزء كاملاً
    // استخدام قارئ الشيخ عبد الرحمن السديس
    const audioUrl = `https://cdn.islamic.network/quran/audio/128/ar.alafasy/${juzNumber}.mp3`;
    ...
    audio.onerror = function() {
        // محاولة استخدام رابط بديل من المصحف المرتل
        status.textContent = '⏳ محاولة رابط بديل...';
        const backupUrl = `https://server8.mp3quran.net/afs/${String(juzNumber).padStart(3, '0')}.mp3`;
        source.src = backupUrl;
        audio.load();
        
        audio.oncanplay = function() {
            status.textContent = '✅ الجزء جاهز للتشغيل (رابط بديل)';
            status.style.color = '#16a34a';
        };
        
        audio.onerror = function() {
            status.textContent = '⚠️ حدث خطأ في تحميل الصوت، حاول مرة أخرى';
            status.style.color = '#dc2626';
            // محاولة ثالثة
            const thirdUrl = `https://server11.mp3quran.net/sds/${String(juzNumber).padStart(3, '0')}.mp3`;
            source.src = thirdUrl;
            audio.load();
        };
    };
...
function getShareText(juzNumber, readJuz = null) {
    ...
    return text.replace(/{juz}/g, juzToShow).replace(/{readJuz}/g, juzToShow);
}
...
function openShareModal() {
    ...
    document.getElementById('shareMessageDisplay').textContent = shareText;
...
async function updateUI() {
    ...
    // عرض الجزء المقروء اليوم إذا كان موجوداً، وإلا عرض الجزء المطلوب
    ...
    for (let m of filtered) {
        ...
        cont.innerHTML += `<div class="admin-list-item" style="${hasReadToday ? 'border-right:4px solid #22c55e;' : 'border-right:4px solid #ef4444;'}">
            <div>
                ...
                <small>📖 الجزء المطلوب: ${m.currentJuz || m.selectedJuz} | الجزء المقروء اليوم: ${readToday}</small><br>
...
function exportReportToExcel() {
    ...
    rows.forEach(row => {
        ...
        cells.forEach((cell, index) => {
            rowData[`عمود ${index + 1}`] = cell.textContent.trim();
        });
    ...
    XLSX.writeFile(wb, `تقرير_ختمتي_${new Date().toLocaleDateString('ar')}.xlsx`);
    showToast('✅ تم تصدير Excel', false);
}
...
document.getElementById('listenJuzBtn')?.addEventListener('click', function() {
    // عرض الجزء المقروء اليوم إذا كان موجوداً
    const readJuz = currentMemberData?.readJuzList || [];
    const today = getTodayString();
    const lastRead = safeDateString(currentMemberData?.lastReadDate);
    const hasReadToday = lastRead === today;
